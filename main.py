...
def func(p):
    x, y = p
    return np.sin(x + y) - 1.2 * x - 0.1, x * x + y * y - 1
...
res_ = newton(x0, y0, newton_func1, newton_func2, e)
result_print(res_)
...
x_gr = np.arange(-0.9, 0, 0.01)
plt.grid(True)
plt.plot(x_gr, f1(x_gr), lw=2, color="pink")
plt.plot(x_gr, -f2(x_gr), lw=2, color="violet")
plt.show()